...
func incorrect_or_timeout():
	...
	if incorrect:
		$error_label.text = "Incorrect! You inputted %s instead of %s! \n Press any button to continue!" % [last_action, current_action]
	else:
		$error_label.text = "Ran out of time!\nYou need to press %s for action \"%s\"!\nPress any button to continue!" %["Button", action_texts.get(current_action)]
	incorrect = false
